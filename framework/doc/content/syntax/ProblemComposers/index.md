...
This custom problem operator example will follow the
[custom_composer_and_operator.i](/test/tests/mfem/problemcomposers/custom_composer_and_operator.i) test which
...
`MooseObject`, thus it has a fixed signature constructor and destructor, plus a `createProblemOperator()`
method it must override. An example minimal class looks like:
...
options associated with it directly (e.g. solver customizations may occur in the `MFEMProblem`
